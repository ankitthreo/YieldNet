...
def get_regularizer_function(regularizer: str) -> nn.Module:
    
    if regularizer == 'zero_reg':
        return lambda P, I: torch.zeros_like(torch.norm(P * (1 - I)).sum())
    elif regularizer == 'squared_fro_norm_inv':
        return lambda P, I: torch.square(torch.norm(P * (1 - I))).sum()
    elif regularizer == 'batched_fro_norm':
        return lambda P, I: torch.norm(P * I, dim = 0).sum()
    elif regularizer == 'batched_fro_norm_inv':
        return lambda P, I: torch.norm(P * (1 - I), dim = 0).sum()
    elif regularizer == 'squared_fro_norm':
        return lambda P, I: torch.square(torch.norm(P * I)).sum()
    elif regularizer == 'fro_norm_inv':
        return lambda P, I: torch.norm(P * (1 - I), dim=[2,3]).sum()
    elif regularizer == 'sqrt_squared_fro_norm_inv':
        return lambda P, I: torch.sqrt(torch.square(torch.norm(P * (1 - I))).sum())
    elif regularizer == 'abs_norm_inv':
        return lambda P, I: torch.abs(P * (1 - I)).sum()
    else:
