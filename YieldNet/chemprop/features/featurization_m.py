...
def set_reaction(reaction: bool, mode) -> None:  ###changed
    """
    ...
    if reaction:
        ...
        #PARAMS.MULTISTEP = multistep ###changed
...
def is_multistep(is_mol: bool = True) -> bool:     ###changed, added a func.
    """Returns whether it's a multistep reaction or not"""
    if is_mol:
        return False
...
def map_reac_to_prod(mol_reac: Chem.Mol, mol_prod: Chem.Mol):
    ...
    return res, only_prod_ids, only_reac_ids, reac_id_to_prod_id #res
...
class MolGraph:
    ...
    * :code:`b2br`: A mapping from f_bonds to real bonds in molecule recorded in targets.
    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        ...
        self.is_multistep = is_multistep(self.is_mol) ###changed
        ...
        if type(mol) == str:
            if self.is_reaction:
                if self.is_multistep:
                    ...
                    step_repo = []
                    for step in steps:
                        step_mol = (make_mol(step.split(">")[0], self.is_explicit_h, self.is_adding_hs, self.is_keeping_atom_map), make_mol(step.split(">")[-1], self.is_explicit_h, self.is_adding_hs, self.is_keeping_atom_map))
                        step_repo.append(step_mol)
                        
                else: 
                    self.n_steps = 1       
                    step_repo = [(make_mol(mol.split(">")[0], self.is_explicit_h, self.is_adding_hs, self.is_keeping_atom_map), make_mol(mol.split(">")[-1], self.is_explicit_h, self.is_adding_hs, self.is_keeping_atom_map))]   ###changed 
                    print("fdhgdg")
            else:
        ...
        if not self.is_reaction:
            ...
            for m in mol:
                ...
                mol_reac = m[0]
                mol_prod = m[1]
                shuffled_ri2pi, pio, rio, original_ri2pi = map_reac_to_prod(mol_reac, mol_prod)
                # Get atom features
                if self.reaction_mode in ['reac_diff','prod_diff', 'reac_prod']:
                    ...
                    self.f_atoms_reac.append(f_atoms_reac)
                    ...
                    f_atoms_prod = [atom_features(atom) for atom in mol_prod.GetAtoms()] + [atom_features_zeros(mol_reac.GetAtomWithIdx(index)) for index in rio]
                    self.f_atoms_prod.append(f_atoms_prod)
                    ...
                    self.f_atoms_prod.append(f_atoms_prod)
                self.perm_mat.append(permutation_mat(len(f_atoms_reac), original_ri2pi, rio, pio)) #, random_perm=True
                
                self.n_atoms = len(f_atoms_reac)
                ...
                self.adj_r.append(Chem.rdmolops.GetAdjacencyMatrix(mol_reac))
                self.adj_p.append(Chem.rdmolops.GetAdjacencyMatrix(mol_prod))
                r_edge_feature = np.zeros((self.n_atoms, self.n_atoms, PARAMS.BOND_FDIM))
                p_edge_feature = np.zeros((self.n_atoms, self.n_atoms, PARAMS.BOND_FDIM))
                # Get bond features
                for a1 in range(self.n_atoms):
                    for a2 in range(a1 + 1, self.n_atoms):
                        if a2 < mol_reac.GetNumAtoms():
                            bond_reac = mol_reac.GetBondBetweenAtoms(a1, a2)
                        else:
                            bond_reac = None
                            
                        if a2 < mol_reac.GetNumAtoms():
                            bond_prod = mol_reac.GetBondBetweenAtoms(a1, a2)
                        else:
                            bond_prod = None
                        
                        f_bond_reac = bond_features(bond_reac)
                        f_bond_prod = bond_features(bond_prod)
                        r_edge_feature[a1, a2, :] = f_bond_reac
                        r_edge_feature[a2, a1, :] = f_bond_reac
                        p_edge_feature[a1, a2, :] = f_bond_prod
                        p_edge_feature[a2, a1, :] = f_bond_prod
                        
                self.r_edge_feature.append(r_edge_feature)
                self.p_edge_feature.append(p_edge_feature)
...
class BatchMolGraph:
    ...
    * :code:`b2br`: (Optional): A mapping from f_bonds to real bonds in molecule recorded in targets.
    ...
    def __init__(self, mol_graphs: List[MolGraph]):
        ...
        self.max_num_atoms = max(1, max(max(len(mol_graph.f_atoms_reac[i]) for i in range(mol_graph.n_steps)) for mol_graph in mol_graphs))
        f_atoms_reac = []
        f_atoms_prod = []
        f_bonds_reac = []
        f_bonds_prod = []
        ...
        perm = []
        num_atoms = []
        for g, mol_graph in enumerate(mol_graphs):
            edge_r = np.zeros((mol_graph.n_steps, self.max_num_atoms, self.max_num_atoms, PARAMS.BOND_FDIM))
            edge_p = np.zeros((mol_graph.n_steps, self.max_num_atoms, self.max_num_atoms, PARAMS.BOND_FDIM))
            A_r = np.zeros((mol_graph.n_steps, self.max_num_atoms, self.max_num_atoms))
            A_p = np.zeros((mol_graph.n_steps, self.max_num_atoms, self.max_num_atoms))
            f_atoms_reac.append([])
            f_atoms_prod.append([])
            num_atoms.append([])
            permstep = []
            for s in range(mol_graph.n_steps):
                num_atoms[g].append(len(mol_graph.f_atoms_reac[s]))
                ...
                f_r = mol_graph.f_atoms_reac[s] + [[0] * PARAMS.ATOM_FDIM] * (self.max_num_atoms - len(mol_graph.f_atoms_reac[s]))
                f_p = mol_graph.f_atoms_prod[s] + [[0] * PARAMS.ATOM_FDIM] * (self.max_num_atoms - len(mol_graph.f_atoms_prod[s]))
                f_atoms_reac[g].append(f_r)
                f_atoms_prod[g].append(f_p)
            
                edge_r[s, :len(mol_graph.r_edge_feature[s]), :len(mol_graph.r_edge_feature[s])] = mol_graph.r_edge_feature[s]
                edge_p[s, :len(mol_graph.p_edge_feature[s]), :len(mol_graph.p_edge_feature[s])] = mol_graph.p_edge_feature[s]
            
                A_r[s, :len(mol_graph.adj_r[s]), :len(mol_graph.adj_r[s])] = mol_graph.adj_r[s]
                A_p[s, :len(mol_graph.adj_p[s]), :len(mol_graph.adj_p[s])] = mol_graph.adj_p[s]
            
                P[:len(mol_graph.perm_mat[s]), :len(mol_graph.perm_mat[s])] = mol_graph.perm_mat[s]
                permstep.append(P)
            f_bonds_reac.append(edge_r)
            f_bonds_prod.append(edge_p)
            adj_reac.append(A_r)
            adj_prod.append(A_p)
            perm.append(permstep)
        ...
        B, S, N, _ = self.f_atoms_reac.shape
        self.indicator = torch.ones((B,S,N,N))
        self.indicator = self.indicator - torch.floor(torch.cdist(self.f_atoms_reac[:,:,:,:100],self.f_atoms_prod[:,:,:,:100],p=4))
