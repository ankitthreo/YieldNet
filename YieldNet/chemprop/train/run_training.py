...
def run_training(args: TrainArgs,
    ...
    for model_idx in range(args.ensemble_size):
        ...
        val_perm_path = "./chemprop/train/val_perm"
        
        for epoch in trange(args.epochs):
            debug(f'Epoch {epoch}')
            model.encoder.set_mode('train')
            n_iter = train(
                model=model,
            ...
            model.encoder.set_mode('val')
            val_perm = np.genfromtxt(f"{val_perm_path}/epoch_{epoch}.txt").tolist()
            val_scores = evaluate(
                model=model,
        ...
        # Evaluate on test set using model last epoch
        info(f'=================================================================')
        info(f'Model on last epoch')
        
        if empty_test_set:
            info(f'Model {model_idx} provided with no test set, no metric evaluation will be performed.')
        else:
            test_preds, _, _, _, _, _, _ = predict(
                model=model,
        ...
        if empty_test_set:
            ...
            test_preds, test_dperms, test_htraces, test_straces, p_gold, p_hard, p_phi = predict(
                model=model,
                data_loader=test_data_loader,
                scaler=scaler,
                atom_bond_scaler=atom_bond_scaler
            ...
            PE = torch.tensor(test_dperms).squeeze(-1).tolist()
            AE = torch.abs(torch.tensor(test_targets)-torch.tensor(test_preds)).squeeze(-1).tolist()
            df = pd.DataFrame({'AE': AE, 'PE': PE, 'hard_traces' : test_htraces, 'soft_traces' : test_straces})
            df.to_csv(os.path.join(args.save_dir, 'interpret.csv'), index=False)
            if args.perm_type == 'soft':
                for i in range(len(p_hard)):
                    np.save(open(f"{args.save_dir}/perms/p_gold_{i}.npy",'wb'), p_gold[i])
                    np.save(open(f"{args.save_dir}/perms/p_hard_{i}.npy",'wb'), p_hard[i])
                    np.save(open(f"{args.save_dir}/perms/p_phi_{i}.npy",'wb'), p_phi[i])
