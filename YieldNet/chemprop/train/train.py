...
def train(
    ...
    for batch in tqdm(data_loader, total=len(data_loader), leave=False):
        ...
        # Run model
        model.zero_grad()
        preds, perm_loss, perm_diff, htraces, straces, p_golds, p_hards, p_phis = model(
            mol_batch,
            features_batch,
        ...
        if model.is_atom_bond_targets:
            ...
            if args.loss_agg_func == 'mean':
                loss = ((preds - targets) ** 2).mean()
            elif args.loss_agg_func == 'sum':
                loss = ((preds - targets) ** 2).sum()
            
            loss += args.perm_reg_lambda * perm_loss
            loss_sum += loss.item()
            iter_count += 1
