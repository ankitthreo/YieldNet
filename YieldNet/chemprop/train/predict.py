...
def predict(
    ...
    for batch in tqdm(data_loader, disable=disable_progress_bar, leave=False):
        ...
        if model.encoder.soft_perm==False and model.encoder.mode == 'test' and model.encoder.args.perm_mode == 'gold':
            gold_test_p = np.load(open(f"{model.encoder.args.gold_path}/p_{model.encoder.args.gold_pname}_{i}.npy",'rb'))
            i += 1
        ...
        if model.encoder.soft_perm==False and model.encoder.mode == 'test' and model.encoder.args.perm_mode == 'gold':
            with torch.no_grad():
                batch_preds, perm_loss, batch_perm_diff, batch_htraces, batch_straces, gold_p, hard_p, perm_phi = model(
                    mol_batch,
                    features_batch,
        ...
        if model.is_atom_bond_targets:
            ...
            batch_perm_diff = batch_perm_diff.data.cpu()
            batch_htraces = batch_htraces.data.cpu()
            batch_straces = batch_straces.data.cpu()
            gold_p = gold_p.data.cpu().numpy()
            hard_p = hard_p.data.cpu().numpy()
            perm_phi = perm_phi.data.cpu().numpy()
            if model.loss_function == "mve":
            ...
            batch_perm_diff = batch_perm_diff.tolist()
            perm_diff.extend(batch_perm_diff)
            batch_htraces = batch_htraces.tolist()
            hsim_trace.extend(batch_htraces)
            batch_straces = batch_straces.tolist()
            ssim_trace.extend(batch_straces)
            p_gold_list.append(gold_p)
            p_hard_list.append(hard_p)
            p_phi_list.append(perm_phi)
    ...
    return preds, perm_diff, hsim_trace, ssim_trace, p_gold_list, p_hard_list, p_phi_list
