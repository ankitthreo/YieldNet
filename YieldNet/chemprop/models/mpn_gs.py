...
class DeepSet(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.inner_LRL = nn.Sequential(
            nn.Linear(input_size, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size)
        )
        
        self.outer_LRL = nn.Sequential(
            nn.Linear(hidden_size, hidden_size),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        spring = self.inner_LRL(x)
        summer = torch.sum(spring, dim=-2)
        autumn = self.outer_LRL(summer)
...
def hard_perm(perm):
    ...
    cost = torch.max(perm) - perm
    #cost = -perm
    row_ind, col_ind = linear_sum_assignment(cost)
    hard_p = torch.zeros_like(perm)
    hard_p[row_ind, col_ind] = 1.0
    return hard_p
...
class MPNEncoder(nn.Module):
    ...
    def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int, hidden_size: int = None,
       ...
       """
        ...
        if self.aggregation == 'deepset':
            self.node_deepset = DeepSet(input_size=self.hidden_size, hidden_size=self.hidden_size, output_size=self.hidden_size)
        ...
        if self.step_aggregation == 'deepset':
            self.deepset = DeepSet(input_size=self.hidden_size, hidden_size=self.hidden_size, output_size=self.hidden_size)
    ...
    def pytorch_attentions(self, log_alpha):
        batch_size = log_alpha.size()[0]
        n = log_alpha.size()[1]
    ...
    def forward(self,
        ...
        if soft_perm==True:
            ...
            for i in range(atom_hiddens_r.shape[1]):
                if self.args.format == "att":
                    cost = torch.einsum('bid,bdj->bij', self.FF_node(atom_hiddens_r[:, i]), self.FF_node(atom_hiddens_p[:, i]).mT)
                    permutation.append(self.pytorch_attentions(cost).unsqueeze(1))
                
                elif self.args.format == "perm":    
                    if self.args.similarity == 'dot':
                        ...
                        cost = - torch.sum(torch.maximum(r_mat, p_mat), dim=-1)
                        permutation.append(self.pytorch_sinkhorn_iters(cost, val_perm, indicator, ptype='node').unsqueeze(1))
        ...
        if soft_perm==False and self.mode == 'test' and self.args.perm_mode == 'gold':
            permutation = torch.from_numpy(val_perm).to(self.device)
        ...
        if self.args.perm_type == 'nopm':
            ...
            if self.mode == 'train':
                rand_idx = torch.randperm(atom_hidden_p.shape[2])
                atom_hidden_p = atom_hidden_p[:, :, rand_idx, :].view(atom_hidden_p.size())
                
            atom_hiddens = atom_hidden_r + atom_hidden_p_
        ...
        elif self.aggregation == 'deepset':
            mol_vecs = self.node_deepset(atom_hiddens)  
        ...
        elif self.step_aggregation == 'deepset':
            mol_vecs = self.deepset(mol_vecs)
            
        else:
            mol_vecs = mol_vecs.sum(1)
        
        if permutation.size(1)==1:
            hard_p = torch.stack(list(map(hard_perm, permutation.squeeze(1).cpu().detach()))).to(self.device)
            hard_sim = (hard_p.mT)@(perm.squeeze(1))
            # soft_sim = (permutation.squeeze(1).mT)@(perm.squeeze(1))
            explicit_sim = (perm.squeeze(1) - hard_p).mT@(indicator.squeeze(1))
            hsim_trace = torch.stack(list(map(torch.trace, hard_sim)))
            # ssim_trace = torch.stack(list(map(torch.trace, soft_sim)))
            ssim_trace = torch.stack(list(map(torch.trace, explicit_sim)))
            dperm = torch.norm((perm-permutation), dim=(2, 3))
            return mol_vecs, self.regularizer(permutation, indicator), dperm, hsim_trace, ssim_trace, perm, hard_p.unsqueeze(1), permutation  # num_molecules x hidden
        else:
            dummy = torch.zeros(mol_vecs.size(0))
            return mol_vecs, self.regularizer(permutation, indicator), dummy, dummy, dummy, dummy, dummy, dummy
...
class MPN(nn.Module):
    ...
    def __init__(self,
        ...
        self.device = args.device
        if (args.perm_type == 'soft'):
            self.soft_perm = True #True False None
        elif (args.perm_type == 'hard'):
            self.soft_perm = False #True False None
        elif (args.perm_type == 'rand'):
            self.soft_perm = None #True False None
        self.args = args
    ...
    def forward(self,
        ...
        output, perm_loss, dperm, hsim_trace, ssim_trace, p_gold, p_hard, p_phi = encodings[0] if len(encodings) == 1 else torch.cat(encodings, dim=1)
        ...
        return output, perm_loss, dperm, hsim_trace, ssim_trace, p_gold, p_hard, p_phi
