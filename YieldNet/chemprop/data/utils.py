...
def filter_invalid_smiles(data: MoleculeDataset) -> MoleculeDataset:
    ...
    return MoleculeDataset([datapoint for datapoint in tqdm(data)
                            ...
                            and all(m.GetNumHeavyAtoms() > 0 for m in datapoint.mol if not isinstance(m, list))    ####changed tuple to list
                            and all((ind_m[0].GetNumHeavyAtoms() + ind_m[0].GetNumHeavyAtoms() > 0 for ind_m in m) for m in datapoint.mol if isinstance(m, list))])  ####changed tuple to list
