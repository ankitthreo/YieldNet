...
import warnings
import dgl
from dgl.nn.pytorch import NNConv, Set2Set, HGTConv
...
class MPNN(nn.Module):
    ...
    def __init__(self, node_in_feats, edge_in_feats, device, hidden_feats = 20,
        ...
        self.hgt = HGTConv(in_size=hidden_feats, head_size=hidden_feats, num_heads=1, num_ntypes=5, num_etypes=5)
    ...
    def forward(self, g):
        ...
        ntype =  torch.ones(node_feats.shape[0], dtype=torch.long).to(self.device) #1
        etype = torch.ones(edge_feats.shape[0], dtype=torch.long).to(self.device)  #1

        node_aggr = [node_feats]        
        for _ in range(self.num_step_message_passing):
            # print(len(node_feats),len(edge_feats), len(hidden_feats))
            node_feats = self.activation(self.hgt(g, node_feats, ntype, etype)).unsqueeze(0)
