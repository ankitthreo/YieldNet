...
class DeepReac(nn.Module):
    ...
    def __init__(self,
        ...
        #self.gnn2 = nn.Sequential(nn.Linear(hidden_feats_1[0]*2, hidden_feats_1[1]), nn.ReLU(), nn.Linear(hidden_feats_1[1], hidden_feats_1[1]))
        self.in_dim = hidden_feats_1[-1]
        #self.digits = CapsuleLayer(in_nodes_dim=self.in_dim, in_nodes=self.num, out_nodes=1, out_nodes_dim=self.out_dim, device=self.device)
        
        self.predict = nn.Sequential(
            nn.Dropout(dropout_2),
            nn.Linear(self.out_dim, 1) #*2
        )
