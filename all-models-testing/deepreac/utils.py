...
def smi2graph(smi):
    if smi != "":
        ...
        g = mol_to_bigraph(mol, node_featurizer=ChempropFeaturizer())
    else:
        g = mol_to_bigraph(None, node_featurizer=ChempropFeaturizer())
    return g
...
def processing(token, raw):
    ...
    if token=='SC':
        reactant = "*".join(reac_list[:4])
        base = ".".join(reac_list[4:])
        intermediate = "*".join(intmt_list[:2])
        hal = ".".join(intmt_list[2:4])
        bor = ".".join(intmt_list[4:-1])
        product = intmt_list[-1]
        
        return "*".join([reactant, base, intermediate, hal, bor, product])
    
    elif token=='NS':
        reactant = "*".join(reac_list)
        int_1 = ".".join(intmt_list[:2])
        int_2 = ".".join(intmt_list[2:-1])
        product = intmt_list[-1]
        
        return "*".join([reactant, int_1, int_2, product])
        
    else: 
        reactant = "*".join(reac_list)
        intermediate = "*".join(intmt_list)
        return "*".join([reactant, intermediate])
...
def load_data(src_file, y_standard=1, datatype='test'):
    df = pd.read_csv(src_file)
    data_token = src_file.split('/')[-1][:2]
    cols = df.columns
    df['output'] = df[cols[-1]]
    raw_list = df[cols[0]]
    
    from functools import partial
    pro = partial(processing, data_token)
    
    if raw_list[0].count('|')>0:
        if 'uspto' in src_file:
            rsmi_list = list(map(processing_uspto, raw_list))
        else:  
            rsmi_list = list(map(pro, raw_list))
    else:
        rsmi_list = raw_list
    df['reaction'] = rsmi_list
    data = []
    c_num = 0
    mean = 0
    std = 0
...
def run_an_eval_epoch_my(model, data_loader, mean, std, args, device):
    model.eval()
    eval_meter = Meter()
    index = []
    outfeat_list = []
