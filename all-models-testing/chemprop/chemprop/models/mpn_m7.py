...
class GraphEncoder(nn.Module):
    ...
    def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int, hidden_size: int = None,
        ...
        self.W_g = nn.Linear(self.hidden_size, self.hidden_size, bias=self.bias)
        
        self.attn_l = nn.Parameter(torch.ones(self.hidden_size)).to(self.device)
        self.attn_r = nn.Parameter(torch.ones(self.hidden_size)).to(self.device)
    ...
    def forward(self, atom_feat, bond_feat, need_message=False):
    
        input_atom = self.W_i_atom(atom_feat)  # num_atoms x hidden_size
        input_atom = self.act_func(input_atom)
        message_atom = input_atom.clone()
        
        input_bond = self.W_i_bond(bond_feat)  # num_bonds x hidden_size
        ...
        for depth in range(self.depth - 1):
            ...
            emb = self.W_g(message_atom)
            l = (emb*(self.attn_l.unsqueeze(dim=0).unsqueeze(dim=0).unsqueeze(dim=0))).sum(-1, keepdim=True).unsqueeze(2).expand(-1, -1, message_bond.shape[2], -1, -1)
            r = (emb*(self.attn_r.unsqueeze(dim=0).unsqueeze(dim=0).unsqueeze(dim=0))).sum(-1, keepdim=True).unsqueeze(3).expand(-1, -1, message_bond.shape[3], -1, -1)
            attention = l + message_bond.sum(-1, keepdim=True) + r
            ...
            message_bond = message_bond*torch.softmax(attention, dim=2)
...
class MPNEncoder(nn.Module):
    ...
    def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int, hidden_size: int = None,
       ...
       """
        ...
        input_dim_bond = self.bond_fdim
        self.mol_encoder = GraphEncoder(input_dim, input_dim_bond, self.hidden_size)
        
        self.condensed_encoder = GraphEncoder(input_dim, input_dim_bond, self.hidden_size)
    ...
    def forward(self,
        ...
        if soft_perm==True:
            ...
            atom_hiddens_r, message_bond_reac = self.mol_encoder(self.f_atoms_reac_sep, self.f_bonds_reac_sep , need_message=True)
            atom_hiddens_p, message_bond_prod = self.mol_encoder(self.f_atoms_prod_sep, self.f_bonds_prod_sep , need_message=True)
            ##upto this
            ...
            for depth in range(self.depth - 1):
                ...
                emb_reac = self.W_g(message_atom_reac)
                l_reac = (emb_reac*(self.attn_l.unsqueeze(dim=0).unsqueeze(dim=0).unsqueeze(dim=0))).sum(-1, keepdim=True).unsqueeze(2).expand(-1, -1, message_bond_reac.shape[2], -1, -1)
                r_reac = (emb_reac*(self.attn_r.unsqueeze(dim=0).unsqueeze(dim=0).unsqueeze(dim=0))).sum(-1, keepdim=True).unsqueeze(3).expand(-1, -1, message_bond_reac.shape[3], -1, -1)
                attention_reac = l_reac + message_bond_reac.sum(-1, keepdim=True) + r_reac
                ...
                message_bond_reac = message_bond_reac*torch.softmax(attention_reac, dim=2)
                ...
                emb_prod = self.W_g(message_atom_prod)
                l_prod = (emb_prod*(self.attn_l.unsqueeze(dim=0).unsqueeze(dim=0).unsqueeze(dim=0))).sum(-1, keepdim=True).unsqueeze(2).expand(-1, -1, message_bond_prod.shape[2], -1, -1)
                r_prod = (emb_prod*(self.attn_r.unsqueeze(dim=0).unsqueeze(dim=0).unsqueeze(dim=0))).sum(-1, keepdim=True).unsqueeze(3).expand(-1, -1, message_bond_prod.shape[3], -1, -1)
                attention_prod = l_prod + message_bond_prod.sum(-1, keepdim=True) + r_prod
                ...
                message_bond_prod = message_bond_prod*torch.softmax(attention_prod, dim=2)
