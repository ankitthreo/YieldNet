    ...
    def forward(self,
        ...
        if soft_perm==True:
            ...
            for depth in range(self.depth - 1):
                ...
                message_atom_reac = message_atom_reac + agg_message_reac#*torch.sigmoid(message_bond_reac.max(dim=2)[0])
                ...
                message_atom_prod = message_atom_reac + agg_message_prod#*torch.sigmoid(message_bond_prod.max(dim=2)[0])
        ...
        for depth in range(self.depth - 1):
            ...
            message_atom = message_atom + agg_message#*torch.sigmoid(message_bond.max(dim=2)[0])
            if soft_perm==True:
