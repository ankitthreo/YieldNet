...
def set_reaction(reaction: bool, multistep:bool, mode: str) -> None:  ###changed
    """
...
def map_reac_to_prod(mol_reac: Chem.Mol, mol_prod: Chem.Mol):
    ...
    only_prod_ids = []
    prod_map_to_id = {}
    #print("mols",  mol_reac, Chem.MolToSmiles(mol_prod)) #, Chem.MolFromSmiles(Chem.MolToSmiles(mol_reac)
        ...
        if not self.is_reaction:
            ...
            for m in mol:
                ...
                if self.reaction_mode in ['reac_diff','prod_diff', 'reac_prod']:
                    ...
                    self.f_atoms_prod.append(f_atoms_prod)
                self.perm_mat.append(permutation_mat(len(f_atoms_reac), ri2pi, rio, pio)) #, random_perm=True
