...
class MoleculeDatapoint:
    ...
    def number_of_steps(self) -> int:
        ...
        """
        return len(self.smiles[0].split('|'))
...
class MoleculeDataset(Dataset):
    ...
    def number_of_steps(self) -> int:
        ...
        :return: The number of molecules.
        """
        return self._data[0].number_of_steps if len(self._data) > 0 else None
    ...
    for s, reaction, keep_h, add_h, keep_atom_map in zip(smiles, reaction_list, keep_h_list, add_h_list, keep_atom_map_list):
        if reaction:
            if s.find('|')==-1:
                ...
                stepmol = []
                for step in s.split('|'):
                    stepmol.append(SMILES_TO_MOL[step] if step in SMILES_TO_MOL else (make_mol(step.split(">>")[0], keep_h, add_h, keep_atom_map), make_mol(step.split(">>")[-1], keep_h, add_h, keep_atom_map)))
                mol.append(stepmol)
        else:
