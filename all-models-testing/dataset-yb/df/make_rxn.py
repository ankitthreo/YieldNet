...
for i in range(1, 11):
    ...
    dfs = pd.read_excel("df_full.xlsx", sheet_name='FullCV_'+sheet_id)
    df_ = generate_dof_mechanism(dfs)
    rxns = []
    for j, row in df_.iterrows():
        
        rxn = f"{row['alcohol']}*{row['base']}*{row['sulfonyl_fluoride']}*{row['oxide']}*{row['conjugate_acid']}*{row['sulfoxide']}*{row['fluoride']}*{row['product']}"
        rxns.append(rxn)
    ...
    train, val, test = df.iloc[:518], df.iloc[518:592], df.iloc[592:]
    df.to_csv(f"./full/DFi_FullCV_{str(i)}.csv", index=False)
    train.to_csv(f"./train/DFi_FullCV_{str(i)}.csv", index=False)
    val.to_csv(f"./val/DFi_FullCV_{str(i)}.csv", index=False)
    test.to_csv(f"./test/DFi_FullCV_{str(i)}.csv", index=False)
